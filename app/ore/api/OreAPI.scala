...
import db.impl.query.{ProjectQueries, VersionQueries}
...
object OreAPI {
  ...
  object v1 {
    ...
    def getProjectList(categories: Option[String], sort: Option[Int], q: Option[String],
      ...
      val queries = service.provide[ProjectQueries]
    ...
    def getVersionList(pluginId: String, channels: Option[String],
      ...
      Project.withPluginId(pluginId).map { project =>
        ...
        val filter = channelIds.map(service.provide[VersionQueries].channelFilter).orNull
    ...
    = Json.toJson(service.await(service.provide[UserQueries].collect(limit.getOrElse(-1), offset.getOrElse(-1))).get)
