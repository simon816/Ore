...
package object schema {
  ...
  def convertUnapply[P <: Product, A, Repr <: HList, Rest <: HList](f: A => Option[P])(
    ...
    val mappedOptProd: Option[toTuple.Out] = optProd.map { prod =>
      ...
      val newGen: Option[ObjectReference] :: Option[Timestamp] :: Rest = id.unsafeToOption :: time.unsafeToOption :: rest
      toTuple(newGen)
    }
...
class TagTable(tag: RowTag) extends ModelTable[ProjectTag](tag, "project_tags") with NameColumn[ProjectTag] {
  ...
  override def * = {
    ...
    val convertedUnapply: PartialFunction[ProjectTag, (Option[ObjectReference], List[ObjectReference], String, String, TagColor)] = {
