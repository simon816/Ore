...
/**
  ...
  * @param colorHex Channel color hex code
  */
case class ChannelData(name: String, private val colorHex: String) {

  /** The [[Channel]] [[Color]] **/
  val color: Color = Channel.Colors.find(_.hex.equalsIgnoreCase(colorHex)).get
  ...
  def addTo(project: Project): Either[String, Channel] = {
    ...
    if (channels.size >= Project.MaxChannels) {
      ...
      channels.find(_.name.equalsIgnoreCase(this.name)) match {
        ...
        case None => channels.find(_.color.equals(this.color)) match {
          ...
          case None => Right(project.addChannel(this.name, this.color))
        }
      }
    }
  }
  ...
  def saveTo(oldName: String)(implicit project: Project): Option[String] = {
    ...
    if (colorTaken) {
      ...
      val nameChan = channels.find(_.name.equalsIgnoreCase(this.name))
